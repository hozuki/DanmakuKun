...
namespace FastColoredTextBoxNS
{
    ...
    public class AutocompleteItem
    {
        ...
        protected static char[] _delimChars = { '.', ' ', '(', ')', '=', ':', '+', '-', '*', '/', '?', ',', '!', '@', '#', '$', '%', '^', '&', '~', '\\', '|', '<', '>', '\n', '\r', '\t', '\v', '\f' };
    ...
    public class SnippetAutocompleteItem : AutocompleteItem
    {
        ...
        {
            e.TextBox.BeginUpdate();
            e.TextBox.Selection.BeginUpdate();
            //remember places
            var p1 = popupMenu.Fragment.Start;
            var p2 = e.TextBox.Selection.Start;
            //do auto indent
            if (e.TextBox.AutoIndent)
            {
                for (int iLine = p1.iLine + 1; iLine <= p2.iLine; iLine++)
                {
                    e.TextBox.Selection.Start = new Place(0, iLine);
                    e.TextBox.DoAutoIndent(iLine);
                }
            }
            e.TextBox.Selection.Start = p1;
            //move caret position right and find char ^
            while (e.TextBox.Selection.CharBeforeStart != '^')
                if (!e.TextBox.Selection.GoRightThroughFolded())
                    break;
            //remove char ^
            e.TextBox.Selection.GoLeft(true);
            e.TextBox.InsertText("");
            //
            e.TextBox.Selection.EndUpdate();
            e.TextBox.EndUpdate();
        }
    ...
    public class MethodAutocompleteItem : AutocompleteItem
    {
        ...
        public override CompareResult Compare(string fragmentText)
        {
            ...
            int i = fragmentText.LastIndexOfAny(_delimChars);
            if (i < 0)
                return CompareResult.Hidden;
            string lastPart = fragmentText.Substring(i + 1);
            //string lastPart = fragmentText;
        ...
        public override string GetTextForReplace()
        {
            ...
            return firstPart + Parent.ReplaceChar.ToString() + Text;
        }
    }
}
